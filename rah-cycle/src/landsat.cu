...
void Landsat::process_final_products(Station station, MTL mtl){
    ...
    while(Erro) {
        ...
        for(int line = 0; line < height_band; line++){
            ...
            save_tiffs(std::vector<double*> {ustar_write_line, aerodynamic_resistance_write_line},
            		std::vector<TIFF*> {ustar_tif1, aerodynamic_resistance_tif1}, line);
    ...
    for(int line = 0; line < height_band; line++){
        ...
        save_tiffs(std::vector<double*> {sensible_heat_flux_line},
        		std::vector<TIFF*> {sensible_heat_flux}, line);
    }
    ...
    for(int line = 0; line < height_band; line++){
        ...
        save_tiffs(std::vector<double*> {latent_heat_flux_line, net_radiation_24h_line, latent_heat_flux_24h_line, evapotranspiration_fraction_line, evapotranspiration_24h_line},
        		std::vector<TIFF*> {latent_heat_flux, net_radiation_24h, latent_heat_flux_24h, evapotranspiration_fraction, evapotranspiration_24h}, line);
