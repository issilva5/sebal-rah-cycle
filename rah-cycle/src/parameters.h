...
struct Sensor{
	 ...
	 */
	std::string capture_parameter_path(int number_sensor, int year);

	/**
	 * @brief  Loads the sensor parameters.
	 * @param  sensor_path: Path to sensor parameter file.
	 */
	void load_parameter_values(std::string sensor_path);
};
...
struct Station {
	 ...
	 */
	Station(std::string station_data_path, double image_hour);
};
